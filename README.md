...
Recommended when the story is non-trivial or the engineer wants a second pair of eyes on the spec before the chain spends tokens on it. Claude reads the rig's existing context — other stories, the build plan, the code shape — and drafts the spec in conversation.
