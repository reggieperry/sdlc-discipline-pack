...
> Elder examples in this guide are illustrative — they show what Freeman & Pryce's
> TDD discipline looks like applied to a non-trivial codebase. The principle
> applies across projects.

A principal-engineer reference for using tests to drive both code correctness and design quality (Elder is the illustrative codebase here). The thesis is Freeman and Pryce's: tests are not merely a verification step — they are the design oracle. Test pain is design feedback; pain ignored is design that won't survive contact with future requirements. The spine of this guide is *Growing Object-Oriented Software, Guided by Tests* (Freeman & Pryce, Addison-Wesley, 2009), with citations by chapter and page where the position depends on the primary text. Where Liskov's modularity guide and the DDD guide already cover a point, this guide cites them rather than restating.
